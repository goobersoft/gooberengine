...
#define actor_paddle_bending_up()    1
#define actor_paddle_bending_none()  0
#define actor_paddle_bending_down() -1
...
#define actor_paddle_bending(self)  get(self,bending)
...
#define actor_paddle_controllable(self) (self->controllable)
#define actor_paddle_active(self)       (self->active)
...
void init_actor_paddle( actor_paddle_t * self ) {
  ...
  actor_paddle_bending(self)        = actor_paddle_bending_none();
  // 1 indicates only one internal segment between the two end pieces.
  actor_paddle_width(self)          = 1;
  actor_paddle_controllable(self)   = false();
  actor_paddle_active(self)         = true();
}
...
void actor_paddle_move_left( actor_paddle_t * self ) {
  if (actor_paddle_active(self)) {
    actor_paddle_pos_x(self) = low(actor_paddle_pos_x(self)-2,100);
  }
}

void actor_paddle_move_right( actor_paddle_t * self ) {
  if (actor_paddle_active(self)) {
    actor_paddle_pos_x(self) = high(actor_paddle_pos_x(self)+2,280-(actor_paddle_width(self)*10));
  }
}

void actor_paddle_bend( actor_paddle_t * self, int d ) {
  d = clamp(d,-1,1);
  actor_paddle_bending(self) = d; 
  // set the sprite accordingly.
  switch(d) {
    case 0:   sprite_set_pos( actor_paddle_sprite(self), 70, 10 ); break;
    case 1:   sprite_set_pos( actor_paddle_sprite(self), 70, 0 );  break;
    case -1:  sprite_set_pos( actor_paddle_sprite(self), 70, 20 ); break;
  }
}

void actor_paddle_bend_down( actor_paddle_t * self ) {
  actor_paddle_bend(self,-1);
}

void actor_paddle_bend_up( actor_paddle_t * self ) {
  actor_paddle_bend(self,1);
}
