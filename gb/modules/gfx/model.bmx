...
function model_draw_wire(m:t_model)
  gb_graph_set_mode(gb_graph_mode_add)
  for local i:int = 0 to m.tris.length-1
    gb_graph_set_color(m.tris[i].color)
    gb_graph_draw_tri_wire(m.tris[i].points[0], m.tris[i].points[1], m.tris[i].points[2])
  next
  gb_graph_reset()
endfunction

function model_draw_wire3d(m:t_model)
  gb_graph_set_mode(gb_graph_mode_add)
  for local i:int = 0 to m.tris.length-1
    gb_graph_set_color(m.tris[i].color)
...
function model_draw(m:t_model)
endfunction

function model_draw3d(m:t_model)
endfunction
