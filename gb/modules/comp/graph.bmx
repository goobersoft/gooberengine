...
include "graph/perspec.bmx"
include "graph/scanlines.bmx"
...
type t_gb_graph
  ...
  field mode            :t_int
  field flip            :t_bool[]
  field center          :t_point
  field zrange          :t_number
  field drawscale       :t_point

  field cameramode      :t_bool
  field pixelmode       :t_bool

  field perspec         :t_gb_graph_perspec
  field scanlines       :t_gb_graph_scanlines
endtype

function new_gb_graph:t_gb_graph()
  ...
  r.offset      = new_point(0,0)
  r.clip        = new_rect(0,0,40,24)
  r.angle       = new_number(0,0,1000,true)
  r.perspec     = new_gb_graph_perspec()
  r.scanlines   = new_gb_graph_scanlines()
  r.zrange      = new_number(0,-23.99,100000)
  r.mode        = new_int(gb_graph_mode_normal)
  r.center      = new_point(20,12)
  r.flip        = new_bool_v(2,false)
  r.pixelmode   = new_bool(false)
  r.cameramode  = new_bool(true)
...
function gb_graph_calc_viewport()
	local cx:float = (gb.graph.clip.w/40*gb.visual.letterbox.rect.w)
	local cy:float = (gb.graph.clip.h/24*gb.visual.letterbox.rect.h)
	setviewport( ..
		roundint(gb.visual.letterbox.rect.x), roundint(gb.visual.letterbox.rect.y),
		roundint(cx), roundint(cy))
endfunction

function gb_graph_calc_perspec:float( z:float )
	if bool_eq(gb.graph.pixelmode) then z = pixel(z)
	local u:float = number_get(gb.graph.perspec.value)
	if z > -u
		return (u / (z+u))
	endif
	return 0.0
...
function gb_graph_calc_offsets()
	local u:float
	if bool_eq(gb.graph.cameramode)
		u = wrap(number_get(gb.graph.angle) - gb.camera.angle.z,0,1000)
	else
		u = number_get(gb.graph.angle)
	endif
	gb.graph.offset.x = (gb.graph.handle.x * cosf(u)) + (gb.graph.handle.y * -sinf(u))
	gb.graph.offset.y = (gb.graph.handle.y * sinf(u)) + (gb.graph.handle.y * cosf(u))
endfunction

function gb_graph_calc_drawscale()
	gb.graph.drawscale.x = float_get(gb.visual.pixelscale) * float_get(gb.visual.virtualscale)
	gb.graph.drawscale.y = float_get(gb.visual.pixelscale) * float_get(gb.visual.virtualscale)
endfunction

function gb_graph_calc_x:float(x:float)
	if bool_eq(gb.graph.cameramode) then x = x - gb.camera.pos.x
	if bool_eq(gb.graph.pixelmode) then x = pixel(x)
	return gb.visual.letterbox.rect.x + ((x-(gb.graph.offset.x)) * gb.graph.drawscale.x * gb_tilesize)
endfunction

function gb_graph_calc_y:float(y:float)
	if bool_eq(gb.graph.cameramode) then y = y - gb.camera.pos.y
	if bool_eq(gb.graph.pixelmode) then y = pixel(y)
	return gb.visual.letterbox.rect.y + ((y-(gb.graph.offset.y)) * gb.graph.drawscale.y * gb_tilesize)
endfunction

function gb_graph_calc_w:float(w:float)
	if bool_eq(gb.graph.pixelmode) then w = pixel(w)
	return w * gb.graph.drawscale.x * gb_tilesize * gb.graph.scale.x
endfunction

function gb_graph_calc_h:float(h:float)
	if bool_eq(gb.graph.pixelmode) then h = pixel(h)
	return h * gb.graph.drawscale.y * gb_tilesize * gb.graph.scale.y 
endfunction
...
function gb_graph_calc_x3d:Float( x:Float, z:Float )
	...
	if bool_eq(gb.graph.pixelmode)
		x = pixel(x)
		z = pixel(z)
	endif
	Return gb.visual.letterbox.rect.x + ..
		((((x-gb.graph.center.x-gb.graph.offset.x) * ..
		gb_graph_calc_perspec(z))+gb.graph.center.x)* ..
		gb.graph.drawscale.x*gb_tilesize)
endfunction
...
function gb_graph_calc_y3d:Float( y:Float, z:Float )
	...
	Return gb.visual.letterbox.rect.y + ..
		((((y-gb.graph.center.y-gb.graph.offset.y) * ..
		gb_graph_calc_perspec(z))+gb.graph.center.y)* ..
		gb.graph.drawscale.y*gb_tilesize)
endfunction

function gb_graph_calc_w3d:Float( w:Float, z:Float )
	if bool_eq(gb.graph.cameramode) then z = z - gb.camera.pos.z
	if bool_eq(gb.graph.pixelmode)
		w = pixel(w)
		z = pixel(z)
	endif
	Return w * gb.graph.drawscale.x * gb_graph_calc_perspec(z) * ..
		gb_tilesize * gb.graph.scale.x
endfunction
...
function gb_graph_calc_h3d:Float( h:Float, z:Float )
	...
	Return h * gb.graph.drawscale.y * gb_graph_calc_perspec(z) * ..
		gb_tilesize * gb.graph.scale.y
endfunction

function gb_graph_within_zrange:Int(z:Float)
	if bool_eq(gb.graph.cameramode) then z = z - gb.camera.pos.z
	If in_range(z, gb.graph.zrange.low, gb.graph.zrange.high) Then Return True
	Return False
endfunction
...
function gb_graph_enable_camera( n:int )
	bool_set( gb.graph.cameramode, n )
endfunction

function gb_graph_set_pixelmode( n:int )
	bool_set( gb.graph.pixelmode, n )
endfunction

function gb_graph_enable_perspec( n:int )
	bool_set( gb.graph.perspec.active, n )
endfunction

function gb_graph_set_perspec( f:float=24.0 )
	number_set(gb.graph.perspec.value,f)
endfunction

function gb_graph_set_center( x:float=20.0, y:float=12.0 )
	point_set(gb.graph.center, x, y)
endfunction

function gb_graph_set_mode( d:int )
	select(d)
		case gb_graph_mode_normal
			int_set( gb.graph.mode, gb_graph_mode_normal )
		case gb_graph_mode_add
			int_set( gb.graph.mode, gb_graph_mode_add )
		case gb_graph_mode_multiply
			int_set( gb.graph.mode, gb_graph_mode_multiply )
		default
			int_set( gb.graph.mode, gb_graph_mode_normal )
	endselect
endfunction
...
function gb_graph_set_color( r:float, g:float, b:float )
	color_set_rgb(gb.graph.color,r,g,b)
endfunction

function gb_graph_set_color( c:t_color )
	gb.graph.color.r = c.r
	gb.graph.color.g = c.g
	gb.graph.color.b = c.b
endfunction

function gb_graph_set_alpha( a:float )
	color_set_alpha( gb.graph.color, a )
endfunction

function gb_graph_set_alpha( c:t_color )
	gb.graph.color.a = c.a
endfunction
...
function gb_graph_set_scale(x:float, y:float)
	point_set(gb.graph.scale,x,y)
	gb_graph_calc_offsets()
endfunction

function gb_graph_set_flip(x:int, y:int)
	bool_set( gb.graph.flip[0], x )
	bool_set( gb.graph.flip[1], y )
endfunction

function gb_graph_set_handle(x:float, y:float)
	point_set(gb.graph.handle,x,y)
	gb_graph_calc_offsets()
endfunction
...
function gb_graph_set_clip(x:float, y:float, w:float, h:float)
	...
	w = clamp(w,0,40.0-x)
	h = clamp(h,0,24.0-y)
	rect_set(gb.graph.clip,x,y,w,h)
endfunction
...
function gb_graph_draw_tile_text(x:Float, y:Float, f:Int, t:String)
	Local ft	:t_font		= gb.assets.fonts[f]
	Local l 	:Int 			= Len(t)
	Local c 	:Byte			= 0
	Local px	:Int			= 0
	Local py	:Int			= 0
