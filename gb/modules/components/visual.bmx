...
global gb_visual_scanlines_active   :t_bool
...
function gb_visual_init()
  ...
  gb_visual_scanlines_active              = new_bool(false)
...
function gb_visual_load()
	...
	bool_set    (gb_visual_scanlines_active,    dict_read_byte(u,"visual/scanlines/active"))
...
function gb_visual_draw(x:float=0, y:float=0)
	if bool_eq(gb_visual_scanlines_active)
...
function gb_visual_end()
	...
	dict_write_byte		(d,"scanlines/active",gb_visual_scanlines_active.value)
	dict_write_byte		(d,"scanlines/thick",byte(gb_visual_scanlines_thick.value))
	dict_write_byte		(d,"scanlines/style",byte(gb_visual_scanlines_style.value))
