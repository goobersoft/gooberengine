...
void debug_load() {
  ...
  debug_colormap = assets_get_colormap(gb_assets(),4);
}
...
void debug_draw_post() {
  ...
  */
  ...
  loop(i,0,10) {
    graph_set_mode( gb_graph(), graph_mode_sub() );
    graph_set_color( gb_graph(), color(1,0,1) );
    graph_draw_hl( gb_graph(), 0, wp, 400 );
    wp = (wp + 1) % 240;
  }
  */
  graph_set_mode           ( gb_graph(), graph_mode_avg() );
  //graph_draw_colormap      ( gb_graph(), 0, 0, debug_colormap );
  graph_draw_colormap_spray( gb_graph(), 0, 0, debug_colormap, 50 );
