...
function new_lightcube:t_lightcube(f:byte=false)
	...
	if bool_eq(r.fine)
		ibox_set(r.ibox, 1,20+ffloor(frand(0,3)),15+ffloor(frand(0,3)),1,1)
	else
		ibox_set(r.ibox, 1,17+ffloor(frand(0,3)),15+ffloor(frand(0,3)),1,1)
	endif
...
function lightcube_update(a:t_lightcube, d:float)
...
function lightcube_draw3d(a:t_lightcube, x:float=0, y:float=0, z:float=0)
	...
	for local i:int = 0 to lightcube_max_points-1
		...
		yy = (i / 10) mod 10
		zz = i mod 10
		drawcmd_set_color( u, 0.5*(float(xx)/10), 0.5*(float(yy)/10), 0.5*(float(zz)/10))
